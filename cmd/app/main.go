...
func main() {
	...
	filmsConn, err := grpc.Dial("sessions:8020", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Fatal(err)
	}

	usersConn, err := grpc.Dial("sessions:8030", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Fatal(err)
	}
	...
	router.HandleFunc("/films/comments/add", filmsPageHandlers.AddComment).Methods("POST", "OPTIONS")
	router.HandleFunc("/films/comments/remove", filmsPageHandlers.RemoveComment).Methods("POST", "OPTIONS")
