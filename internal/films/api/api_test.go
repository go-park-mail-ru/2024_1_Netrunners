...
func TestFilmsServer_GetAllFilmComments(t *testing.T) {
	...
	mockService.EXPECT().GetAllFilmComments(ctx, filmUuid, userUuid).Return(expectedComments, nil)

	req := &session.AllFilmCommentsRequest{FilmUuid: filmUuid, UserUuid: userUuid}
