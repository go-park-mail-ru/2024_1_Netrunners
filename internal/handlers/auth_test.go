...
func TestAuthPageHandlers_Signup(t *testing.T) {
	validCases := []struct {
		...
		{
			"valid json",
			[]byte(`{"login": "for_tests_only_login@mail.ru", "username": "Danya", "password": "12345678"}`),
			"for_tests_only_login@mail.ru",
			"Danya",
			"12345678",
		},
	}
	...
	authStorage, err := database.NewUsersStorage()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	authPageHandlers := NewAuthPageHandlers(authService, sessionService, sugarLogger)
...
func TestAuthPageHandlers_Login(t *testing.T) {
	validCases := []struct {
		...
		{
			"valid json",
			[]byte(`{"login": "for_tests_only_login@mail.ru", "password": "12345678"}`),
			"for_tests_only_login@mail.ru",
			"12345678",
		},
	}
	...
	invalidCases := []struct {
		...
		{
			"wrong password",
			[]byte(`{"login": "for_tests_only_login@mail.ru", "password": "12345677"}`),
		},
	}
...
func TestAuthPageHandlers_Logout(t *testing.T) {
	...
	reqLogin, _ := http.NewRequest("POST", "/auth/login", bytes.NewBuffer([]byte(
		`{"login": "for_tests_only_login@mail.ru", 
		 "password": "12345678"}`)))
	handlerLogin := http.HandlerFunc(authPageHandlers.Login)
	...
	err = authPageHandlers.sessionService.HasSession("for_tests_only_login@mail.ru", refreshToken)
	if err == nil {
